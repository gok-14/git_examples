import requests
import time
from database_old import print_db, init_db
...
API_KEY = os.getenv("API_KEY")  # ← must match .env key name

# -----------------------------
# Send Single Location
# -----------------------------
def send_location(owner_id, vehicle_id, driver_name, latitude, longitude, speed, status):
    try:
...
def simulate_moving(owner_id):
    ...
    for i, point in enumerate(route):
        ...
        send_location(
            owner_id, "TRK-001", "John Doe",
            point["lat"], point["lng"],
            point["speed"], point["status"]
        )
        time.sleep(5)
...
if __name__ == "__main__":
    ...
    if not API_KEY:
        print("❌ INTERNAL_API_KEY not found in .env — check your .env file!")
        exit()

    print(f"✅ API Key loaded: {API_KEY[:6]}******")  # shows first 6 chars only
    ...
    while True:
        ...
        if choice == "1":
            owner = input("Owner ID (e.g. owner001): ").strip()
            send_location(owner, "TRK-001", "John Doe", 13.7563, 100.5018, 60, "moving")

        elif choice == "2":
            owner = input("Owner ID (e.g. owner001): ").strip()
            simulate_moving(owner)
